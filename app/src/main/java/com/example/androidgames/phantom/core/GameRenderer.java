...
import com.example.androidgames.phantom.core.tile.Tile;
...
public class GameRenderer {
    ...
    public void draw(Canvas g, TileMap mainMap, TileMap backgroundMap,
        ...
        Integer newOffsetX = 80;

        if (drawHudEnabled) {
            drawStringDropShadowAsHud(g, "MARIONETTE x " + Settings.getLives(), 8, 4, 0, 1);
            drawStringDropShadowAsHud(g, df2.format(Settings.getScore()), 8, 20, 0, 1);

            g.drawBitmap(MarioResourceManager.Coin_Icon, 108 + newOffsetX, 4, null);
            drawStringDropShadowAsHud(g, "x " + Settings.getCoins(), 120 + newOffsetX, 3, 0, 1);
            //drawStringDropShadow(g,"x "+Settings.getCoins(),9,0);
            drawStringDropShadowAsHud(g, "WORLD", 170 + newOffsetX, 4, 0, 1);
            drawStringDropShadowAsHud(g, Settings.world + "-" + Settings.level, 170 + newOffsetX, 20, 0, 1);
