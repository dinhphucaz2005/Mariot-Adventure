...
public class LevelCompleteScreen extends Screen {
 	...
 	public void loadGame() {
  		try {
			...
			if (map.bookMarks().size()>=Settings.level){
				...
				pearls.add(new Point((int)(mario.getX()),(int) mario.getY()));//+phantom.getHeight()/2)));
			}else{
	...
	private void drawRunningUI() {
		...
		/*
		for (int i=0; i<phantom.getX(); i+=MarioResourceManager.pearl1.getWidth()+3){
			gameCanvas.drawBitmap(bmp,i,phantom.getY()+2*phantom.getHeight(),null);
		}
		*/
