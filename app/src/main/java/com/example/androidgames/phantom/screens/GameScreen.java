...
import com.example.androidgames.phantom.core.GameLoader;
...
public class GameScreen extends Screen {
	...
	public void loadGame() {
		...
		try {
			...
			//phantom.setAlive(true);
			//phantom.reloadAnimation();
			//phantom.setX(32);
			//phantom.setY(1);
			//phantom.setdY(0.1f);
			...
			eventID = 0;
			map.setPlayer(mario); // set the games main player to phantom
			Settings.setPlayer(mario);
	    	Settings.setLives(mario.getHealth());
	    	Settings.setTime((int) timeRemaining);
	...
	private void updateRunning(List<TouchEvent> touchEvents, float deltaTime) {
		...
		if (isSystemDriven){
			//phantom.setY(phantom.getY()+0.1f);
			//phantom.setdY(0);
			mario.setX(32);
		...
		if (!mario.isAlive()) {
			...
			if (mario.getY() > map.getHeight() * 16 + mario.getHeight()) {
				...
				if (mario.getHealth() >= 0 && timeRemaining >0) {
					...
					//Settings.setLives(phantom.getHealth())
					//phantom.update(map, period, true);
					//return;
				} else {
		...
		map.creaturesToAdd().clear(); // This line MUST be called BEFORE
										...
										// as a result of phantom colliding are
