package com.example.androidgames.phantom.objects.mario;
...
import com.example.androidgames.phantom.core.animation.CollidableObject;
import com.example.androidgames.phantom.core.tile.GameTile;
import com.example.androidgames.phantom.core.tile.TileMap;
import com.example.androidgames.phantom.objects.base.Collision;
import com.example.androidgames.phantom.objects.base.Creature;
import com.example.androidgames.phantom.objects.creatures.BoomRang;
import com.example.androidgames.phantom.objects.creatures.Bowser;
import com.example.androidgames.phantom.objects.creatures.Coin;
import com.example.androidgames.phantom.objects.creatures.FireBall;
import com.example.androidgames.phantom.objects.creatures.FireDisc;
import com.example.androidgames.phantom.objects.creatures.FireFlower;
import com.example.androidgames.phantom.objects.creatures.FlyGoomba;
import com.example.androidgames.phantom.objects.creatures.FlyRedKoopa;
import com.example.androidgames.phantom.objects.creatures.Goomba;
import com.example.androidgames.phantom.objects.creatures.JumpingFish;
import com.example.androidgames.phantom.objects.creatures.Latiku;
import com.example.androidgames.phantom.objects.creatures.LevelComplete;
import com.example.androidgames.phantom.objects.creatures.Mushroom;
import com.example.androidgames.phantom.objects.creatures.Platform;
import com.example.androidgames.phantom.objects.creatures.RedFish;
import com.example.androidgames.phantom.objects.creatures.RedKoopa;
import com.example.androidgames.phantom.objects.creatures.RedShell;
import com.example.androidgames.phantom.objects.creatures.Score;
import com.example.androidgames.phantom.objects.creatures.Spring;
import com.example.androidgames.phantom.objects.creatures.Thorny;
import com.example.androidgames.phantom.objects.tiles.Brick;
import com.example.androidgames.phantom.objects.tiles.FireTile;
import com.example.androidgames.phantom.objects.tiles.QuestionBlock;
import com.example.androidgames.phantom.objects.tiles.RotatingBlock;
import com.example.androidgames.phantom.particles.BubbleParticle;
import com.example.androidgames.phantom.particles.SmokeParticle;
import com.example.androidgames.phantom.particles.SparkleParticle;
...
public class Mario extends CollidableObject{
	 ...
	 * Start_Run_Anim_Thres: The speed where phantom switches to the running animation.
	 */
	...
	/*boolean which keeps track of direction of phantom*/
    private boolean left=false;
    /*boolean which keeps track of size of phantom*/
    ...
    //boolean to keep track if phantom is selfDriven (at stagebegin and end)
	...
	/**
	 * Debugging method used to print the status of phantom with regards to platforms.
	 */ 
	...
	/**
	 * Fixes Y movement on tiles and platforms where animation height changes by setting the phantom's y
	...
	public void update(TileMap map, float time,boolean lockInput) {
		...
		if(oldX < 0) { // Collision with left side of map.
			...
			if(numOfXTiles == 0) { // No tile collision in the X direction
				...
				if(!isAbovePlatform) { setdX(0); } // Stop movement only if phantom isn't on a Platform
												   // Why do this? If I don't phantom gets frozen to tiles
		...
		super.update((int) time); // Update phantom's animation.
		
		// Manage collision in the Y direction. 
		boolean upperCollision = false; // will check if phantom is above a tile
	...
	// called from within marioUpdate() when phantom hits a block while moving up.
	...
	// Determines what happens when phantom collides with a creature.
    ...
    public void processEvent(int eventID) {
        ...
        if(eventID == 0) {
        	if (inWater){
        		...
        		isJumping = true;
        		System.out.println("jump phantom touchevent");
        		return;
        	}else if(!isJumping) {
        		//isSpaceHeld = true;
